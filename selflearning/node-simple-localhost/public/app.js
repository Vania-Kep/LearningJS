...
var config = {
  issuer: 'https://allergan-val.oktapreview.com/oauth2/default',
  clientId: '0oa2fx3soklo1q4Wb0h8',
...
function loadConfig() {
  ...
  var redirectUri = 'http://localhost:8080/login/callback'; // window.location.origin + '/login/callback'; // Should also be set in Okta Admin UI
...
function showSigninWidget(options) {
  ...
  options = Object.assign({
    ...
    useClassicEngine: !config.useInteractionCodeFlow, // use classic engine for authn
    state: JSON.stringify(config.state),
    authParams: {
      issuer: config.issuer,
      pkce: false,
...
const oktaDomain = 'https://allergan-val.oktapreview.com';
//   options = {
//             el: '#signin-widget',
//             baseUrl: config.issuer.split('/oauth2')[0],
//             clientId: config.clientId,
...
//             // For session token flow, we don't need redirectUri in the widget config
//             authParams: {
